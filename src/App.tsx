...
import {
  ...
  connectSession, pollStatus, sendHeartbeat, disconnectLocal,
...
function App() {
  ...
  // 'lemonade' = one-click polling, 'fallback' = device-code entry
  const [overlayMode, setOverlayMode] = useState<'lemonade' | 'fallback'>('lemonade');
  ...
  // Unified "we're connected" handler — works for both modes.
  ...
  // Legacy: called when a token is obtained via device-code exchange.
  ...
  // --- Bootstrap connection on load ---
  // Tries Lemonade mode first (auto-poll for plugin session).
  // If a legacy device-code token is already in sessionStorage, use it directly.
  // If no plugin appears within ~10s, the overlay switches to fallback mode
  // so the user can enter a code manually.
  useEffect(() => {
    let cancelled = false;
    let lemonadeTimeout: ReturnType<typeof setTimeout> | null = null;

    async function bootstrap() {
      // 1. Legacy token present? Skip Lemonade polling.
      const existingToken = getToken();
      if (existingToken) {
        onPaired(existingToken);
        return;
      }

      // 2. Try Lemonade mode: register our site session, then poll for plugin.
      setPairingStatus('disconnected');
      setOverlayMode('lemonade');
      setStatusText('Looking for plugin…');

      try {
        await connectSession();
      } catch {
        setStatusText('Broker unreachable. Retrying in 5s…');
        if (!cancelled) setTimeout(bootstrap, 5000);
        return;
      }
      if (cancelled) return;

      // Heartbeat for our site session
      if (heartbeatTimerRef.current) clearInterval(heartbeatTimerRef.current);
      ...
      statusTimerRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (s.connected && !wasConnected) {
            ...
            setPairingStatus('disconnected');
            setOverlayMode('lemonade');
            setStatusText('Plugin disconnected — waiting…');
          } else if (!s.connected && !wasConnected) {
            setStatusText('Waiting for plugin to connect…');
            // After ~20s of no plugin, switch to fallback so the user can enter a code
            if (pollCount > 10 && overlayMode === 'lemonade') {
              setOverlayMode('fallback');
              setStatusText('No plugin detected — enter a pairing code');
            }
          }
        } catch {
    ...
    return () => {
      cancelled = true;
      if (lemonadeTimeout) clearTimeout(lemonadeTimeout);
      if (statusTimerRef.current) clearInterval(statusTimerRef.current);
      if (heartbeatTimerRef.current) clearInterval(heartbeatTimerRef.current);
    };
  }, []);
  ...
  return (
    <div className="flex h-screen bg-[var(--chat-panel)] overflow-hidden">
      ...
      {/* Connection overlay — Lemonade one-click (default) or device-code fallback */}
      <AnimatePresence>
          ...
          <ConnectScreen
            status={
              overlayMode === 'fallback'
                ? 'fallback'
                : statusText.startsWith('Broker unreachable')
                ? 'waiting'
                : 'connecting'
            }
            statusText={pairingError || statusText}
            robloxUsername={account?.roblox_username as string | undefined}
            robloxAvatarUrl={account?.roblox_profile_image as string | undefined}
            pairingCode={pairingCode}
            pairingError={pairingError}
            onPairingCodeSubmit={(code) => {
              setPairingCode(code);
              setExchangeAttempts(1);
              exchangeCodeHandler(code);
            }}
            ...
            onSwitchToFallback={() => {
              setOverlayMode('fallback');
              setPairingError('');
              setStatusText('Enter an 8-character code from the AgileBot plugin');
            }}
          />
        )}
      </AnimatePresence>
